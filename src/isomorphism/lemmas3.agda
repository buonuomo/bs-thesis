...
weakSubst : forall {Γ σ τ} -> (x : Var Γ τ) -> (t : Tm (Γ - x) σ) -> (u : Tm (Γ - x) τ) -> hsubst.subst (wkTm x t) x u ≡ t
...
substVarCommAux2 : forall {Γ σ τ} -> (x : Var Γ σ) -> (u₁ : Tm (Γ - x) σ) -> (y : Var (Γ - x) τ) -> (u₂ : Tm ((Γ - x) - y) τ) -> eq (rem x y) (rem x y) ≡ same -> ! conExc x y >₁ hsubst.subst u₁ y u₂ ≡ substVar (rem x y) (rem x y) (! conExc x y >₁ hsubst.subst u₁ y u₂)
substVarCommAux2 x u₁ y u₂ p with eq (rem x y) (rem x y)
substVarCommAux2 x u₁ y u₂ refl | .same = refl


substVarCommAux1 : forall {Γ σ τ} -> (x : Var Γ σ) -> (u₁ : Tm (Γ - x) σ) -> (y : Var (Γ - x) τ) -> (u₂ : Tm ((Γ - x) - y) τ) -> eq (wkv x y) (wkv (wkv x y) (rem x y)) ≡ diff (wkv x y) (rem x y) -> ! conExc x y >₁ hsubst.subst u₁ y u₂ ≡ hsubst.subst (substVar (wkv (wkv x y) (rem x y)) (wkv x y) (wkTm (rem x y) (! conExc x y >₁ u₂))) (rem x y) (! conExc x y >₁ hsubst.subst u₁ y u₂)
...
substVarCommAux4 : forall {Γ σ τ} -> (x : Var Γ τ) -> (u₁ : Tm (Γ - x) τ) -> (y : Var (Γ - x) σ) -> (u₂ : Tm ((Γ - x) - y) σ) -> eq (wkv x y) (wkv x y) ≡ same -> ! conExc x y >₁ u₂ ≡ hsubst.subst (substVar (wkv x y) (wkv x y) (wkTm (rem x y) (! conExc x y >₁ u₂))) (rem x y) (! conExc x y >₁ hsubst.subst u₁ y u₂)
...
substVarCommAux6 : forall {Γ σ τ ρ} -> (x : Var Γ τ) -> (u₁ : Tm (Γ - x) τ) -> (y : Var (Γ - x) ρ) -> (u₂ : Tm ((Γ - x) - y) ρ) -> (v : Var ((Γ - x) - y) σ) -> eq (rem x y) (wkv (rem x y) (! conExc x y >₀ v)) ≡ diff (rem x y) (! conExc x y >₀ v) -> ! conExc x y >₁ var v ≡ substVar (wkv (rem x y) (! conExc x y >₀ v)) (rem x y) (! conExc x y >₁ hsubst.subst u₁ y u₂)
...
substVarCommAux3 : forall {Γ σ τ ρ} -> (x : Var Γ τ) -> (v : Var (Γ - x) σ) -> (u₁ : Tm (Γ - x) τ) -> (y : Var (Γ - x) ρ) -> (u₂ : Tm ((Γ - x) - y) ρ) -> ! conExc x y >₁ (substVar v y u₂) ≡ hsubst.subst (substVar (wkv x v) (wkv x y) (wkTm (rem x y) (! conExc x y >₁ u₂))) (rem x y) (! conExc x y >₁ hsubst.subst u₁ y u₂)
...
substVarComm : forall {Γ σ τ ρ} -> (v : Var Γ σ) -> (x : Var Γ τ) -> (u₁ : Tm (Γ - x) τ) -> (y : Var (Γ - x) ρ) -> (u₂ : Tm ((Γ - x) - y) ρ) -> ! conExc x y >₁ hsubst.subst (substVar v x u₁) y u₂ ≡ hsubst.subst (substVar v (wkv x y) (wkTm (rem x y) (! conExc x y >₁ u₂))) (rem x y) (! conExc x y >₁ hsubst.subst u₁ y u₂)
...
substComm : forall {σ Γ τ ρ} -> (t : Tm Γ σ) -> (x : Var Γ τ) -> (u₁ : Tm (Γ - x) τ) -> (y : Var (Γ - x) ρ) -> (u₂ : Tm ((Γ - x) - y) ρ) -> ! conExc x y >₁ hsubst.subst (hsubst.subst t x u₁) y u₂ ≡ hsubst.subst (hsubst.subst t (wkv x y) (wkTm (rem x y) (! conExc x y >₁ u₂))) (rem x y) (! conExc x y >₁ hsubst.subst u₁ y u₂)
...
mutual
  ...
  weakSubstNfAux : forall {Γ σ τ} -> (x : Var Γ τ) -> (v : Var (Γ - x) σ) -> (us : Sp (Γ - x) σ ○) -> (t : Nf (Γ - x) τ) -> eq x (wkv x v) ≡ diff x v -> ((ne (wkv x v , wkSp x us)) [ x := t ]) ≡ ne (v , us)
  weakSubstNfAux x v us t p with eq x (wkv x v)
  weakSubstNfAux x v us t refl | .(diff x v) = cong (λ ts → ne (v , ts)) (weakSubstSp x us t)


  weakSubstNf : forall {Γ σ τ} -> (x : Var Γ τ) -> (u : Nf (Γ - x) σ) -> (t : Nf (Γ - x) τ) -> (wkNf x u) [ x := t ] ≡ u
  weakSubstNf x (λn u) t = cong λn (weakSubstNf (vs x) u (wkNf vz t))
  weakSubstNf x (ne (v , us)) t = weakSubstNfAux x v us t (substDiff3 x v)


  weakSubstSp : forall {Γ σ τ ρ} -> (x : Var Γ τ) -> (us : Sp (Γ - x) σ ρ) -> (t : Nf (Γ - x) τ) -> (wkSp x us) < x := t > ≡ us
