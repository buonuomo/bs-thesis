...
mutual
  ...
  wkNfExc : forall {Γ σ τ ρ} -> (x : Var Γ σ) -> (y : Var (Γ - x) τ) -> (t : Nf ((Γ - x) - y) ρ) -> wkNf (wkv x y) (wkNf (rem x y) (! conExc x y >₂ t)) ≡ wkNf x (wkNf y t)
  ...
  wkSpExc : forall {Γ σ τ ρ α} -> (x : Var Γ σ) -> (y : Var (Γ - x) τ) -> (ts : Sp ((Γ - x) - y) ρ α) -> wkSp (wkv x y) (wkSp (rem x y) (! conExc x y >₃ ts)) ≡ wkSp x (wkSp y ts)
...
wkTmSubstExc : forall {Γ σ τ ρ} -> (y : Var Γ σ) -> (t : Tm (Γ - y) τ) -> (x : Var (Γ - y) ρ) -> (u : Tm ((Γ - y) - x) ρ) -> wkTm (rem y x) (! conExc y x >₁ (hsubst.subst t x u)) ≡ hsubst.subst (wkTm y t) (wkv y x) (wkTm (rem y x) (! conExc y x >₁ u))
...
mutual
  ...
  nappWkNf : forall {Γ σ τ ρ} -> (x : Var Γ σ) -> (t₁ : Nf (Γ - x) (τ ⇒ ρ)) -> (t₂ : Nf (Γ - x) τ) -> wkNf x (napp t₁ t₂) ≡ napp (wkNf x t₁) (wkNf x t₂)
  nappWkNf x (λn t) u = wkNfSubstNfExc (vs x) t vz u


  -- ◇ and wk commute

  appNfWkNfSp : forall {Γ σ τ ρ} -> (x : Var Γ σ) -> (u : Nf (Γ - x) τ) -> (ts : Sp (Γ - x) τ ρ) -> (wkNf x u) ◇ (wkSp x ts) ≡ wkNf x (u ◇ ts)
  ...
  wkNfSubstNfExcAux1 : forall {Γ σ τ} -> (y : Var Γ σ) -> (ts : Sp (Γ - y) τ ○) -> (x : Var (Γ - y) τ) -> (u : Nf ((Γ - y) - x) τ) -> eq (wkv y x) (wkv y x) ≡ same -> wkNf (rem y x) (! conExc y x >₂ (u ◇ (ts < x := u >))) ≡ (ne (wkv y x , wkSp y ts)) [ (wkv y x) := (wkNf (rem y x) (! conExc y x >₂ u)) ]
  ...
  wkNfSubstNfExcAux2 : forall {Γ σ τ ρ} -> (y : Var Γ σ) -> (ts : Sp (Γ - y) τ ○) -> (x : Var (Γ - y) ρ) -> (u : Nf ((Γ - y) - x) ρ) -> (v : Var ((Γ - y) - x) τ) -> eq (wkv y x) (wkv (wkv y x) (wkv (rem y x) (! conExc y x >₀ v))) ≡ diff (wkv y x) (wkv (rem y x) (! conExc y x >₀ v)) -> wkNf (rem y x) (! conExc y x >₂ ne (v , (ts < x := u >))) ≡ (ne (wkv (wkv y x) (wkv (rem y x) (! conExc y x >₀ v)) , wkSp y ts)) [ (wkv y x) := (wkNf (rem y x) (! conExc y x >₂ u)) ]
  ...
  wkNfSubstNfExc : forall {Γ σ τ ρ} -> (y : Var Γ σ) -> (t : Nf (Γ - y) τ) -> (x : Var (Γ - y) ρ) -> (u : Nf ((Γ - y) - x) ρ) -> wkNf (rem y x) (! conExc y x >₂ (t [ x := u ])) ≡ (wkNf y t) [ (wkv y x) := (wkNf (rem y x) (! conExc y x >₂ u)) ]
  ...
  wkSpSubstSpExc : forall {Γ σ τ ρ α} -> (y : Var Γ σ) -> (ts : Sp (Γ - y) τ ρ) -> (x : Var (Γ - y) α) -> (u : Nf ((Γ - y) - x) α) -> wkSp (rem y x) (! conExc y x >₃ (ts < x := u >)) ≡ (wkSp y ts) < (wkv y x) := (wkNf (rem y x) (! conExc y x >₂ u)) >
