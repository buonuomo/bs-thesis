...
data _βη-≡_ {Γ : Con} : {σ : Ty} → Tm Γ σ → Tm Γ σ → Set where
  ...
  beta : forall {σ τ} → {t : Tm (Γ , σ) τ} → {u : Tm Γ σ} → app (Λ t) u βη-≡ hsubst.subst t vz u
...
congSubst : forall {σ Γ τ} → (t : Tm Γ τ) → (x : Var Γ σ) → {u₁ u₂ : Tm (Γ - x) σ} → u₁ βη-≡ u₂ → hsubst.subst t x u₁ βη-≡ hsubst.subst t x u₂
...
congEmbSp : forall {Γ σ τ} → (us : Sp Γ σ τ) → {acc₁ acc₂ : Tm Γ σ} → acc₁ βη-≡ acc₂ → embSp us acc₁ βη-≡ embSp us acc₂
congEmbSp ε p = p
...
mutual
  ...
  wkNfTm : forall {Γ σ τ} → (x : Var Γ σ) → (u : Nf (Γ - x) τ) → ⌈ wkNf x u ⌉ βη-≡ wkTm x ⌈ u ⌉
  ...
  wkSpTm : forall {Γ σ τ ρ} → (x : Var Γ σ) → (ns : Sp (Γ - x) τ ρ) → (acc : Tm (Γ - x) τ) → embSp (wkSp x ns) (wkTm x acc) βη-≡ wkTm x (embSp ns acc)
  wkSpTm x ε _ = brefl
...
mutual
  ...
  compVar : forall {σ Γ} → (v : Var Γ σ) → ⌈ nvar v ⌉ βη-≡ var v
...
mutual
  ...
  substEmbSp : forall {Γ σ τ ρ} → (ts : Sp Γ τ ρ) → (x : Var Γ σ) → (t : Nf (Γ - x) σ) → (acc : Tm Γ τ) → embSp (ts < x := t >) (hsubst.subst acc x ⌈ t ⌉) βη-≡ hsubst.subst (embSp ts acc) x ⌈ t ⌉
  ...
  substNfSubst : forall {σ Γ τ} → (t : Nf Γ τ) → (x : Var Γ σ) → (u : Nf (Γ - x) σ) → ⌈ t [ x := u ] ⌉ βη-≡ hsubst.subst ⌈ t ⌉ x ⌈ u ⌉
...
completeness : forall {σ Γ} → (t : Tm Γ σ) → ⌈ nf t ⌉ βη-≡ t
...
convertnf : forall {Γ σ} → (t u : Tm Γ σ) → nf t ≡ nf u → t βη-≡ u
...
mutual
  ...
  wkSpAppSp : forall {Γ σ τ ρ α} → (x : Var Γ σ) → (ts : Sp (Γ - x) τ (ρ ⇒ α)) → wkSp (vs x) (appSp (wkSp vz ts) (nvar vz)) ≡ appSp (wkSp vz (wkSp x ts)) (nvar vz)
  ...
  ne2nfWkNf : forall {ρ Γ σ τ} → (x : Var Γ σ) → (v : Var (Γ - x) τ) → (ts : Sp (Γ - x) τ ρ) → wkNf x (ne2nf (v , ts)) ≡ ne2nf (wkv x v , wkSp x ts)
...
nfWkNfWkTm : forall {Γ σ τ} → (x : Var Γ σ) → (u : Tm (Γ - x) τ) → wkNf x (nf u) ≡ nf (wkTm x u)
...
appNfSpNapp : forall {Γ σ τ ρ} → (u : Nf Γ σ) → (ts : Sp Γ σ (τ ⇒ ρ)) → (t : Nf Γ τ) → u ◇ (appSp ts t) ≡ napp (u ◇ ts) t
...
substAppSp : forall {Γ σ τ ρ α} → (ts : Sp Γ σ (τ ⇒ ρ)) → (t : Nf Γ τ) → (x : Var Γ α) → (u : Nf (Γ - x) α) → (appSp ts t) < x := u > ≡ appSp (ts < x := u >) (t [ x := u ])
...
ne2nfSubstAux : forall {Γ σ τ} → (x : Var Γ σ) → (y : Var (Γ - x) τ) → (ts : Sp Γ τ ○) → (u : Nf (Γ - x) σ) → eq x (wkv x y) ≡ diff x y → (ne2nf (wkv x y , ts)) [ x := u ] ≡ ne2nf (y , (ts < x := u >))
...
mutual
  ...
  ne2nfSubst : forall {ρ Γ σ τ} → (x : Var Γ σ) → (y : Var (Γ - x) τ) → (ts : Sp Γ τ ρ) → (u : Nf (Γ - x) σ) → (ne2nf (wkv x y , ts)) [ x := u ] ≡ ne2nf (y , (ts < x := u >))
  ...
  weakSubstSpApp : forall {Γ σ τ ρ α} → (ts : Sp Γ σ (τ ⇒ α)) → (x : Var Γ ρ) → (u : Nf (Γ - x) ρ) → (appSp (wkSp vz ts) (nvar vz)) < (vs x) := (wkNf vz u) > ≡ appSp (wkSp vz (ts < x := u >)) (nvar vz)
...
concatSp : forall {Γ σ τ ρ} → Sp Γ σ τ → Sp Γ τ ρ → Sp Γ σ ρ
concatSp ε us = us
...
concat1 : forall {Γ σ τ} → (ts : Sp Γ σ τ) → concatSp ts ε ≡ ts
concat1 ε = refl
concat1 (t , ts) = cong (λ us → (t , us)) (concat1 ts)


concat2 : forall {Γ σ τ ρ α} → (ts : Sp Γ σ (τ ⇒ ρ)) → (u : Nf Γ τ) → (us : Sp Γ ρ α) → concatSp ts (u , us) ≡ concatSp (appSp ts u) us
concat2 ε _ _ = refl
...
ne2nfAppNfAux : forall {Γ σ} → (x : Var Γ σ) → (ts : Sp Γ σ ○) → (u : Nf (Γ - x) σ) → eq x x ≡ same → (ne2nf (x , ts)) [ x := u ] ≡ u ◇ (ts < x := u >)
...
mutual
  ...
  {-# TERMINATING #-} -- get rid of this
  appNfNe2nfConcat : forall {Γ σ τ} → (x : Var Γ σ) → (ts : Sp Γ σ τ) → (us : Sp Γ τ ○) → (ne2nf (x , ts)) ◇ us ≡ ne (x , concatSp ts us)
  ...
  substNfNe : forall {Γ σ τ} → (i : Var Γ τ) → (x : Var (Γ - i) σ) → (xs : Sp (Γ - i) σ ○) → (j : Var Γ τ) → (k : Var (Γ - j) τ) → (p : onediff j i) → (l : Var Γ σ) → l ≡ wkv i x → wkv i (! sym (onediffMinus i j p) >₀ k) ≡ j → (ne (l , wkSp i xs)) [ j := (nvar k) ] ≡ ne ((! onediffMinus i j p >₀ x) , (! onediffMinus i j p >₃ xs))
  ...
  substNfSp : forall {Γ σ τ ρ} → (i j : Var Γ σ) → (xs : Sp (Γ - i) ρ τ) → (k : Var (Γ - j) σ) → (p : onediff j i) → wkv i (! sym (onediffMinus i j p) >₀ k) ≡ j → (wkSp i xs) < j := (nvar k) > ≡ ! onediffMinus i j p >₃ xs
  ...
  substNfEq : forall {σ Γ τ} → (i : Var Γ τ) → (n : Nf (Γ - i) σ) → (j : Var Γ τ) → (k : Var (Γ - j) τ) → (p : onediff j i) → wkv i (! sym (onediffMinus i j p) >₀ k) ≡ j → (wkNf i n) [ j := (nvar k) ] ≡ ! onediffMinus i j p >₂ n
  ...
  etaEq : forall {σ Γ τ} → (u : Nf Γ (σ ⇒ τ)) → λn (napp (wkNf vz u) (nvar vz)) ≡ u
  etaEq (λn u) = cong λn (substNfEq (vs vz) u vz vz odz refl)


  ne2nfAppNf : forall {τ Γ σ} → (x : Var Γ σ) → (ts : Sp Γ σ τ) → (u : Nf (Γ - x) σ) → (ne2nf (x , ts)) [ x := u ] ≡ u ◇ (ts < x := u >)
...
nfSubstNf : forall {Γ σ τ} → (t : Tm Γ τ) → (x : Var Γ σ) → (u : Tm (Γ - x) σ) → (nf t) [ x := (nf u) ] ≡ nf (hsubst.subst t x u)
...
soundness : forall {Γ σ} → {t u : Tm Γ σ} → t βη-≡ u → nf t ≡ nf u
...
mutual
  ...
  nfEmbSpAppNf : forall {Γ σ} → (ts : Sp Γ σ ○) → (acc : Tm Γ σ) → nf (embSp ts acc) ≡ (nf acc) ◇ ts
  ...
  stability : forall {Γ σ} → (u : Nf Γ σ) → nf ⌈ u ⌉ ≡ u
